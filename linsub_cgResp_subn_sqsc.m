function [resps] = linsub_cgResp_subn_sqsc(stimstem,sig_n,sig_m,varargin)
% assumes rdim = 2
...
if sdim > 1
    ...
    ON_resps1 = resprange1.*normcdf(ONsub_resps,mean(ONsub_resps),std(ONsub_resps)) + min(ONsub_resps); %cumulative gaussian
    ...
    OFF_resps1 = resprange2.*normcdf(OFFsub_resps,mean(OFFsub_resps),std(OFFsub_resps)) + min(OFFsub_resps); %cumulative gaussian
